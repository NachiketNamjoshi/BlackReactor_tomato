...
## AnyKernel setup
# EDIFY properties
kernel.string=BlackReactor Kernel - Nachiket.Namjoshi 
do.devicecheck=1
do.initd=1
do.modules=1
do.cleanup=1
...
found=$(find init.rc -type f | xargs grep -oh "import /init.reactor.rc");
if [ "$found" != 'import /init.reactor.rc' ]; then
	#append the new lines for this option at the bottom
        echo "" >> init.rc
	echo "import /init.reactor.rc" >> init.rc
fi
